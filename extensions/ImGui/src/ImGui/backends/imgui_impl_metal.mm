...
// axmol fork notes
// -----------------------------------------------------------------------------
// Forked from Dear ImGui v1.92.9 (github.com/ocornut/imgui). Two additive changes,
// stock Init/Shutdown/RenderDrawData API is unchanged:
//
//   1) Context indirection. Renderer state moved from io.BackendRendererUserData
//      to a file-local current-context pointer, with explicit Create/Destroy/
//      Set/GetCurrentContext entry points. Lets multiple parallel Metal renderer
//      states coexist within a single ImGui context (main viewport + N secondary
//      viewports, each with its own MetalContext / pipeline cache / samplers).
//
//   2) Optional texture resolver. SetContextTextureResolver installs a per-context
//      hook that maps ImTextureID -> id<MTLTexture> before binding, so callers
//      can pass foreign handles (e.g. ax::Texture2D*) instead of raw MTLTexture
//      pointers. Fallback path is the stock __bridge cast.
//
// Companion axmol-side glue: imgui_impl_axmol_mtl.mm.
// -----------------------------------------------------------------------------
...
struct ImGui_ImplMetal_Data
{
    ...
    ImGui_ImplMetal_ResolveTextureFn TextureResolver;
    bool                             OwnedByInit; // true if allocated by ImGui_ImplMetal_Init (drives Shutdown teardown)

    ImGui_ImplMetal_Data()      { memset((void*)this, 0, sizeof(*this)); }
};

// axmol multi-viewport extension: current renderer context is tracked here so multiple parallel
// Metal renderer states can coexist within one ImGui context (main viewport + N secondary viewports).
static ImGui_ImplMetal_Data*    g_metalCurrentContext = nullptr;

static ImGui_ImplMetal_Data*    ImGui_ImplMetal_GetBackendData()    { return g_metalCurrentContext; }
static void                     ImGui_ImplMetal_DestroyBackendData(){ ImGui_ImplMetal_DestroyContext((void*)g_metalCurrentContext); }
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                {
                    id<MTLTexture> mtl_tex = bd->TextureResolver
                        ? bd->TextureResolver(tex_id)
                        : (__bridge id<MTLTexture>)(void*)(intptr_t)(tex_id);
                    [commandEncoder setFragmentTexture:mtl_tex atIndex:0];
                }
...
// axmol multi-viewport extension: context indirection
void* ImGui_ImplMetal_CreateContext(id<MTLDevice> device)
{
...
void ImGui_ImplMetal_DestroyContext(void* ctx)
{
    ImGui_ImplMetal_Data* bd = (ImGui_ImplMetal_Data*)ctx;
    if (bd == nullptr)
        return;

    // Scope current-context to this ctx so pipeline/sampler teardown targets the right MetalContext.
    ImGui_ImplMetal_Data* saved = g_metalCurrentContext;
    g_metalCurrentContext = bd;
    [bd->SharedMetalContext.renderPipelineStateCache removeAllObjects];
    bd->SharedMetalContext.samplerStateLinear = nil;
    bd->SharedMetalContext.samplerStateNearest = nil;
    bd->SharedMetalContext.depthStencilState = nil;
    bd->SharedMetalContext = nil;
    g_metalCurrentContext = (saved == bd) ? nullptr : saved;

    IM_DELETE(bd);
}

void ImGui_ImplMetal_SetCurrentContext(void* ctx)
{
    g_metalCurrentContext = (ImGui_ImplMetal_Data*)ctx;
}

void* ImGui_ImplMetal_GetCurrentContext()
{
    return (void*)g_metalCurrentContext;
}

void ImGui_ImplMetal_SetContextTextureResolver(void* ctx, ImGui_ImplMetal_ResolveTextureFn fn)
{
    if (ImGui_ImplMetal_Data* bd = (ImGui_ImplMetal_Data*)ctx)
        bd->TextureResolver = fn;
}
...
bool ImGui_ImplMetal_Init(id<MTLDevice> device)
{
    ...
    ImGui_ImplMetal_Data* bd = (ImGui_ImplMetal_Data*)ImGui_ImplMetal_CreateContext(device);
    bd->OwnedByInit = true;
    ImGui_ImplMetal_SetCurrentContext(bd);
...
{
    ImGuiIO& io = ImGui::GetIO();
    ImGui_ImplMetal_Data* bd = (ImGui_ImplMetal_Data*)io.BackendRendererUserData;
    IM_ASSERT(bd != nullptr && "No renderer backend to shutdown, or already shutdown?");
    IM_ASSERT(bd->OwnedByInit && "BackendRendererUserData was not set by ImGui_ImplMetal_Init");

    ImGui_ImplMetal_SetCurrentContext(bd);
    ImGui_ImplMetal_DestroyDeviceObjects();
    ImGui_ImplMetal_DestroyContext(bd);
